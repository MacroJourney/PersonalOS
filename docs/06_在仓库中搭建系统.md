...
touch 01_Shared/x/rules/_index.md
...
**CLAUDE.md 应包含**：身份定义、四区结构、权限边界、核心准则（两条规则）、复利机制位置、三个工作模式的触发时机、启动 / 结束流程，以及 Inbox 分流最小规范（触发条款、标准版报告字段、仅建议不执行边界、确认门槛）。
...
> 模板覆盖：四区结构、权限边界、核心准则、复利机制、三个工作模式、启动 / 结束流程、Inbox 分流最小规范。内容见 `templates/CLAUDE.template.md`。

**建议初始化一条 Inbox 规则**：在 `01_Shared/x/rules/inbox-triage.md` 新建基础规则，至少包含触发条件、判定信号、低置信度处理、标准版报告字段。这样从第一天开始就有统一分流基线。
